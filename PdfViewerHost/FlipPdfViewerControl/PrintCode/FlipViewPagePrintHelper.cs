...
namespace FlipPdfViewerControl.PrintCode
{
	/// <summary>
	/// Photo size options
	/// </summary>
	...
	class FlipViewPagePrintHelper : PrintHelper
	{
		...
		protected override void PrintTaskRequested(Windows.Graphics.Printing.PrintManager sender, Windows.Graphics.Printing.PrintTaskRequestedEventArgs e)
		{
			...
			printTask = e.Request.CreatePrintTask("FlipViewPdfViewer", sourceRequestedArgs =>
			{
				...
				{
					await hostControl.Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, () =>
					{
						ClearPageCollection();
		...
		private async void PrintDetailedOptionsOptionChanged(PrintTaskOptionDetails sender, PrintTaskOptionChangedEventArgs args)
		{
			...
			{
				await hostControl.Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, printDocument.InvalidatePreview);
			}
		}
		...
		private async Task<UIElement> GeneratePageAsync(int photoNumber, PageDescription pageDescription)
		{
			...
			// get the image from the hostControl
			BitmapImage bitmap = await ((FlipPdfViewerControl)hostControl).GetPdfImageForPrint((uint)photoNumber - 1);
